...
impl LaneKernel<Indexed<NodeIndex, Unit>, Single> for EdgesOperation {
    ...
    fn execute<'a>(
        ...
        let edges: Vec<_> = direction
            .edges_for_node(graphrecord, node)
            .collect::<Distinct<_>>()
            .into();
    ...
    for EdgesOperation
{
    ...
    fn execute<'a>(
        ...
        for value in values {
            let node = value.1?;
            edges.extend(direction.edges_for_node(graphrecord, *node.address()));
        }
...
impl<E: Build<EdgesOperation>> Edges for E {
    type Output = E::Output;

    fn edges(&self, direction: EdgeDirection) -> Self::Output {
        self.build(EdgesOperation { direction })
    }
}
