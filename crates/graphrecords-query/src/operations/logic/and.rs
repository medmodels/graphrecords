...
use crate::{
    And, Explain, IndexDomain, Indexed, Labeled, Mask, Operand, OrderState, QueryResult,
...
impl<I: IndexDomain, M, O: OrderState> EstimateCost<AndOperation<M>> for BoolMaskOperand<I, O>
where
    for<'a> M: ArgumentSource<Keyed<I>, Value<'a> = bool>,
{
...
impl<I: IndexDomain, M, O: OrderState> BitAnd<M> for BoolMaskOperand<I, O>
where
    for<'a> M: ArgumentSource<Keyed<I>, Value<'a> = bool>,
{
    type Output = Self;
