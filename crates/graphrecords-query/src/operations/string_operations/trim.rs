...
impl<I: IndexDomain, V: ValueString> ElementKernel<Indexed<I, V>> for TrimOperation {
    ...
    fn pipeline<'a>(
        ...
        Ok(string_rebuild_map_indexed::<I, V>(
            graphrecord,
            |value, _| Ok(value.trim().to_string()),
            Self::LABEL,
        ))
    }
...
impl<V: ValueString + BareValueDomain> ElementKernel<Bare<V>> for TrimOperation {
    type Emission = Preserving;
...
impl<E: Build<TrimOperation>> Trim for E {
    type Output = E::Output;

    fn trim(&self) -> Self::Output {
        self.build(TrimOperation)
    }
}
