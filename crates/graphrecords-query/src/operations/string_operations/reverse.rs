...
impl<I: IndexDomain, V: ValueString> ElementKernel<Indexed<I, V>> for ReverseOperation {
    ...
    fn pipeline<'a>(
        ...
        Ok(string_rebuild_map_indexed::<I, V>(
            graphrecord,
            |value, _| Ok(value.chars().rev().collect()),
            Self::LABEL,
        ))
    }
...
impl<V: ValueString + BareValueDomain> ElementKernel<Bare<V>> for ReverseOperation {
    type Emission = Preserving;
...
impl<E: Build<ReverseOperation>> Reverse for E {
    type Output = E::Output;

    fn reverse(&self) -> Self::Output {
        self.build(ReverseOperation)
    }
}
