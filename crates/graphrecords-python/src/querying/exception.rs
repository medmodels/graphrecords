...
use graphrecords_query::{
    ...
    error::{
        ...
        grouping::{
            InvalidPartitionBucketArity, MissingGroupAggregate, UnresolvedBucketFailures,
            UnresolvedGroupKeyFailures,
        },
        index::{DuplicateExpandedChildIndex, DuplicateIndex, NoChildIndex},
        numeric::{
            IntegerOverflow, InvalidClipBounds, NegativeLength, NegativeSquareRoot,
            NonIntegerValue, NonNumericValue, NonPositiveLogarithm,
        },
...
create_exception!(graphrecords, ArgumentAbsentError, QueryError);
...
create_exception!(graphrecords, NegativeLengthError, QueryError);
...
impl FailureConversion for Failure {
    fn to_python_error(&self) -> PyErr {
        ...
        match self.kind().name() {
            name if name == <InvalidMedianValue>::name() => {
                InvalidMedianValueError::new_err(message)
            }
