...
impl<O: RootOperand> AttributesTreeContext<O> {
    ...
    {
        match self {
            Self::Operand(operand) => Ok(Box::new(operand.get_attributes(graphrecord)?)),
...
impl<T: GetAttributes> GetAttributes for &T {
    ...
    ) -> GraphRecordResult<AttributesView<'b>> {
...
impl GetAllAttributes<NodeIndex> for NodeOperand {
        ...
        Self: 'a,
    {
        ...
        Ok(Self::get_attributes_from_indices(graphrecord, node_indices))
    }
...
impl GetAllAttributes<EdgeIndex> for EdgeOperand {
        ...
        NodeOperand: 'a,
    {
        ...
        Ok(Self::get_attributes_from_indices(graphrecord, edge_indices))
    }
    ...
    where
        EdgeIndex: 'a,
    {
        EdgeOperation::get_attributes(graphrecord, indices)
    }
}
