...
use crate::{
    ...
    graphrecord::{
        ...
        querying::{
            ...
            operand_traits::{
                Add, Contains, Count, EitherOr, EndsWith, EqualTo, Exclude, GreaterThan,
                GreaterThanOrEqualTo, IsIn, IsMax, IsMin, IsNotIn, LessThan, LessThanOrEqualTo,
                Lowercase, Max, Min, NotEqualTo, Random, Slice, StartsWith, ToValues, Trim,
                TrimEnd, TrimStart, Uppercase,
            },
            values::{
                MultipleValuesWithIndexContext, MultipleValuesWithIndexOperand,
                SingleValueWithoutIndexContext, SingleValueWithoutIndexOperand,
            },
        },
    },
};
...
impl<V: Into<GraphRecordAttribute>> From<V> for SingleAttributeComparisonOperand {
...
impl<V: Into<GraphRecordAttribute>> From<Vec<V>> for MultipleAttributesComparisonOperand {
    fn from(value: Vec<V>) -> Self {
        Self::Attributes(value.into_iter().map(Into::into).collect())
    }
}

impl<V: Into<GraphRecordAttribute>> From<HashSet<V>> for MultipleAttributesComparisonOperand {
...
impl<V: Into<GraphRecordAttribute> + Clone, const N: usize> From<[V; N]>
    for MultipleAttributesComparisonOperand
{
    ...
    pub(crate) fn evaluate_backward(
        ...
        Ok(match self {
            Self::NodeMultipleAttributesWithIndexOperand(operand) => operand
                ...
                .collect(),
            Self::NodeMultipleAttributesWithoutIndexOperand(operand) => {
                operand.evaluate_backward(graphrecord)?.collect()
            }
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for AttributesTreeOperand<O> {
    type InputValue = BoxedIterator<'a, (&'a O::Index, Vec<GraphRecordAttribute>)>;
    type ReturnValue = BoxedIterator<'a, (&'a O::Index, Vec<GraphRecordAttribute>)>;
    ...
    fn evaluate_forward(
        ...
        self.operations
            ...
            .try_fold(attributes, |attribute_tuples, operation| {
                operation.evaluate(graphrecord, attribute_tuples)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateForwardGrouped<'a> for AttributesTreeOperand<O> {
    fn evaluate_forward_grouped(
        ...
        self.operations
            .iter()
            .try_fold(attributes, |attribute_tuples, operation| {
                operation.evaluate_grouped(graphrecord, attribute_tuples)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateBackward<'a> for AttributesTreeOperand<O> {
    ...
    fn evaluate_backward(
        ...
        let attributes: BoxedIterator<_> = Box::new(self.context.get_attributes(graphrecord)?);

        self.evaluate_forward(graphrecord, attributes)
    }
}
...
impl<O: RootOperand> Count for AttributesTreeOperand<O> {
    ...
    fn count(&mut self) -> Wrapper<Self::ReturnOperand> {
        let operand = Wrapper::<Self::ReturnOperand>::new(
            MultipleValuesWithIndexContext::AttributesTreeOperand(self.deep_clone()),
        );

        self.operations
            .push(AttributesTreeOperation::CountOperation {
                operand: operand.clone(),
            });
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for MultipleAttributesWithIndexOperand<O> {
    type InputValue = BoxedIterator<'a, (&'a O::Index, GraphRecordAttribute)>;
    type ReturnValue = BoxedIterator<'a, (&'a O::Index, GraphRecordAttribute)>;
...
impl<'a, O: 'a + RootOperand> EvaluateBackward<'a> for MultipleAttributesWithIndexOperand<O> {
    ...
    fn evaluate_backward(
        ...
        let attributes = self.context.get_attributes(graphrecord)?;
...
impl<O: RootOperand> Count for MultipleAttributesWithIndexOperand<O> {
    ...
    fn count(&mut self) -> Wrapper<Self::ReturnOperand> {
        let operand = Wrapper::<Self::ReturnOperand>::new(
            SingleValueWithoutIndexContext::MultipleAttributesWithIndexOperand(self.deep_clone()),
        );

        self.operations
            .push(MultipleAttributesWithIndexOperation::CountOperation {
                operand: operand.clone(),
            });
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for MultipleAttributesWithoutIndexOperand<O> {
    type InputValue = BoxedIterator<'a, GraphRecordAttribute>;
    type ReturnValue = BoxedIterator<'a, GraphRecordAttribute>;
...
impl<O: RootOperand> Count for MultipleAttributesWithoutIndexOperand<O> {
    ...
    fn count(&mut self) -> Wrapper<Self::ReturnOperand> {
        ...
        self.operations
            .push(MultipleAttributesWithoutIndexOperation::CountOperation {
                operand: operand.clone(),
            });
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for SingleAttributeWithIndexOperand<O> {
    type InputValue = Option<(&'a O::Index, GraphRecordAttribute)>;
    type ReturnValue = Option<(&'a O::Index, GraphRecordAttribute)>;
    ...
    fn evaluate_forward(
        ...
        self.operations
            ...
            .try_fold(attribute, |attribute, operation| {
                operation.evaluate(graphrecord, attribute)
            })
    }
}
    ...
    fn evaluate_forward_grouped(
        ...
        self.operations
            ...
            .try_fold(attributes, |attributes, operation| {
                operation.evaluate_grouped(graphrecord, attributes)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateBackward<'a> for SingleAttributeWithIndexOperand<O> {
    ...
    fn evaluate_backward(
        ...
        let attributes = self.context.evaluate_backward(graphrecord)?;

        let attribute = self.reduce_input(attributes)?;

        self.evaluate_forward(graphrecord, attribute)
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for SingleAttributeWithoutIndexOperand<O> {
    type InputValue = Option<GraphRecordAttribute>;
    type ReturnValue = Option<GraphRecordAttribute>;
