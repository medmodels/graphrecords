...
impl<V: Into<GraphRecordValue>> From<V> for SingleValueComparisonOperand {
...
impl<V: Into<GraphRecordValue>> From<Vec<V>> for MultipleValuesComparisonOperand {
    fn from(value: Vec<V>) -> Self {
        Self::Values(value.into_iter().map(Into::into).collect())
    }
}

impl<V: Into<GraphRecordValue> + Clone, const N: usize> From<[V; N]>
...
impl MultipleValuesComparisonOperand {
    pub(crate) fn evaluate_backward(
        &self,
        graphrecord: &GraphRecord,
    ) -> GraphRecordResult<Vec<GraphRecordValue>> {
        Ok(match self {
    ...
    fn evaluate_forward(
        ...
        self.operations
            ...
            .try_fold(values, |value_tuples, operation| {
                operation.evaluate(graphrecord, value_tuples)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateForwardGrouped<'a> for MultipleValuesWithIndexOperand<O> {
    fn evaluate_forward_grouped(
        ...
        self.operations
            .iter()
            .try_fold(values, |value_tuples, operation| {
                operation.evaluate_grouped(graphrecord, value_tuples)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateBackward<'a> for MultipleValuesWithIndexOperand<O> {
    ...
    fn evaluate_backward(
        ...
        self.evaluate_forward(graphrecord, Box::new(values))
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for MultipleValuesWithoutIndexOperand<O> {
    type InputValue = BoxedIterator<'a, GraphRecordValue>;
    type ReturnValue = BoxedIterator<'a, GraphRecordValue>;
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for SingleValueWithIndexOperand<O> {
    ...
    fn evaluate_forward(
        ...
        self.operations.iter().try_fold(value, |value, operation| {
            operation.evaluate(graphrecord, value)
        })
    }
}
    ...
    fn evaluate_forward_grouped(
        ...
        self.operations
            ...
            .try_fold(values, |values, operation| {
                operation.evaluate_grouped(graphrecord, values)
            })
    }
}
...
impl<'a, O: 'a + RootOperand> EvaluateForward<'a> for SingleValueWithoutIndexOperand<O> {
    type InputValue = Option<GraphRecordValue>;
    type ReturnValue = Option<GraphRecordValue>;
