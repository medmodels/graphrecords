...
macro_rules! get_median {
    ($values:ident, $variant:ident) => {
        if $values.len() % 2 == 0 {
            ...
            let first = GraphRecordValue::$variant(*first);
            let second = GraphRecordValue::$variant(*second);

            first.add(second).unwrap().div(GraphRecordValue::Int(2))
        } else {
            let middle = $values.len() / 2;

            Ok(GraphRecordValue::$variant(
...
impl<O: RootOperand> MultipleValuesWithIndexOperation<O> {
        ...
        O: 'a,
    {
        Ok(match self {
            Self::ValueWithIndexOperation { operand } => {
                Self::evaluate_value_with_index_operation(graphrecord, values, operand)?
            }
            Self::ValueWithoutIndexOperation { operand } => {
                Self::evaluate_value_without_index_operation(graphrecord, values, operand)?
            }
            Self::SingleValueComparisonOperation { operand, kind } => {
            ...
            Self::BinaryArithmeticOperation { operand, kind } => Box::new(
                Self::evaluate_binary_arithmetic_operation(graphrecord, values, operand, kind)?,
            ),
            ...
            Self::EitherOr { either, or } => {
                Self::evaluate_either_or(graphrecord, values, either, or)?
            }
            Self::Exclude { operand } => Self::evaluate_exclude(graphrecord, values, operand)?,
        ...
        O: 'a,
    {
        ...
        Ok(values.collect::<GraphRecordResult<Vec<_>>>()?.into_iter())
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::String(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::Int(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::Float(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::Bool(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::DateTime(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::Duration(_)))
    }
    ...
    where
        O: 'a,
    {
        values.filter(|(_, value)| matches!(value, GraphRecordValue::Null))
    }
        ...
        O: 'a,
    {
        ...
        let either_values = either.evaluate_forward(graphrecord, Box::new(values_1))?;
...
impl<O: RootOperand> MultipleValuesWithIndexOperation<O> {
        ...
        O: 'a,
    {
        Ok(match self {
            Self::ValueWithIndexOperation { operand } => Box::new(
                Self::evaluate_value_with_index_operation_grouped(graphrecord, values, operand)?,
            ),
            Self::ValueWithoutIndexOperation { operand } => Box::new(
                Self::evaluate_value_without_index_operation_grouped(graphrecord, values, operand)?,
            ),
            ...
            Self::EitherOr { either, or } => {
                Self::evaluate_either_or_grouped(graphrecord, values, either, or)?
            }
            Self::Exclude { operand } => {
                Self::evaluate_exclude_grouped(graphrecord, values, operand)?
            }
            Self::Merge { operand } => {
        ...
        O: 'a,
    {
        ...
        let values_1 =
            operand.evaluate_forward_grouped(graphrecord, Box::new(values_1.into_iter()))?;
        ...
        O: 'a,
    {
        ...
        let either_values = either.evaluate_forward_grouped(graphrecord, values_1)?;
        let mut or_values: Vec<_> = or
            .evaluate_forward_grouped(graphrecord, values_2)?
            .collect();
        ...
        O: 'a,
    {
        ...
        let mut result: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, values_1)?
            .collect();
...
impl<O: RootOperand> MultipleValuesWithoutIndexOperation<O> {
    pub(crate) fn evaluate<'a>(
        &self,
        graphrecord: &'a GraphRecord,
        values: impl Iterator<Item = GraphRecordValue> + 'a,
        ...
        O: 'a,
    {
        Ok(match self {
            Self::ValueOperation { operand } => {
                Self::evaluate_value_operation(graphrecord, values, operand)?
            }
            Self::SingleValueComparisonOperation { operand, kind } => {
                Self::evaluate_single_value_comparison_operation(
                    graphrecord,
    ...
    pub(crate) fn get_max(
        mut values: impl Iterator<Item = GraphRecordValue>,
    ...
    pub(crate) fn get_mean(
        ...
        let (sum, count) = values.try_fold((first_value, 1), |(sum, count), value| {
            ...
            match sum.add(value) {
                ...
                Err(_) => Err(GraphRecordError::QueryError(format!(
    ...
    pub(crate) fn get_median(
        ...
        let median = match first_value {
            GraphRecordValue::Int(value) => {
                let mut values: Vec<_> = values.map(|value| {
                    let data_type = DataType::from(&value);

                    match value {
                        GraphRecordValue::Int(value) => Ok(value as f64),
                        GraphRecordValue::Float(value) => Ok(value),
                        _ => Err(GraphRecordError::QueryError(format!(
            ...
            GraphRecordValue::DateTime(value) => {
                let mut values: Vec<_> = values.map(|value| {
                    ...
                    match value {
                        GraphRecordValue::DateTime(naive_date_time) => Ok(naive_date_time),
                        _ => Err(GraphRecordError::QueryError(format!(
                            "Cannot calculate median of mixed data types {first_data_type} and {data_type}. Consider narrowing down the values using .is_int(), .is_float(), .is_datetime() or .is_duration()"
                        ))),
                    }
            ...
            GraphRecordValue::Duration(value) => {
                let mut values: Vec<_> = values.map(|value| {
                    ...
                    match value {
                        GraphRecordValue::Duration(naive_date_time) => Ok(naive_date_time),
                        _ => Err(GraphRecordError::QueryError(format!(
                            "Cannot calculate median of mixed data types {first_data_type} and {data_type}. Consider narrowing down the values using .is_int(), .is_float(), .is_datetime() or .is_duration()"
                        ))),
                    }
    ...
    pub(crate) fn get_std(
        ...
        let GraphRecordValue::Float(variance) = variance else {
            unreachable!()
        };

        Ok(Some(GraphRecordValue::Float(variance.sqrt())))
    }
    ...
    pub(crate) fn get_var(
        ...
        let values = values_2
            ...
            .map(|value| {
                ...
                GraphRecordValue::Int(value) => Ok(value as f64),
                GraphRecordValue::Float(value) => Ok(value),
                _ => Err(GraphRecordError::QueryError(
                    format!("Cannot calculate variance of data type {data_type}. Consider narrowing down the values using .is_int() or .is_float()"),
                )),
            }})
            .collect::<GraphRecordResult<Vec<_>>>()?;
        ...
        Ok(Some(GraphRecordValue::Float(variance)))
    }

    #[inline]
    pub(crate) fn get_count(values: impl Iterator<Item = GraphRecordValue>) -> GraphRecordValue {
        GraphRecordValue::Int(values.count() as i64)
    }
...
impl<O: RootOperand> SingleValueWithIndexOperation<O> {
        ...
        O: 'a,
    {
        ...
        Ok(match self {
            Self::SingleValueComparisonOperation { operand, kind } => {
                Self::evaluate_single_value_comparison_operation(graphrecord, value, operand, kind)?
            }
            Self::MultipleValuesComparisonOperation { operand, kind } => {
                Self::evaluate_multiple_values_comparison_operation(
                    graphrecord,
            ...
            Self::BinaryArithmeticOperation { operand, kind } => {
                Self::evaluate_binary_arithmetic_operation(graphrecord, value, operand, kind)?
            }
            Self::UnaryArithmeticOperation { kind } => {
            ...
            Self::EitherOr { either, or } => {
                Self::evaluate_either_or(graphrecord, value, either, or)?
            }
            Self::Exclude { operand } => {
                match operand.evaluate_forward(graphrecord, Some(value.clone()))? {
                    Some(_) => None,
                    None => Some(value),
                }
            }
    ...
    where
        O: 'a,
    {
        let either_result = either.evaluate_forward(graphrecord, Some(value.clone()))?;
        let or_result = or.evaluate_forward(graphrecord, Some(value))?;
...
impl<O: RootOperand> SingleValueWithIndexOperation<O> {
        ...
        O: 'a,
    {
        ...
        let either_values = either.evaluate_forward_grouped(graphrecord, Box::new(values_1))?;
        let mut or_values: Vec<_> = or
            .evaluate_forward_grouped(graphrecord, Box::new(values_2))?
            .collect();
        ...
        O: 'a,
    {
        ...
        let mut result: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, Box::new(values_1))?
            .collect();
...
impl<O: RootOperand> SingleValueWithoutIndexOperation<O> {
    ...
    #[inline]
    fn evaluate_slice(value: GraphRecordValue, range: &Range<usize>) -> GraphRecordValue {
        value.slice(range.clone())
    }

    #[inline]
    fn evaluate_is_string(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::String(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_int(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::Int(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_float(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::Float(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_bool(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::Bool(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_datetime(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::DateTime(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_duration(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::Duration(_) => Some(value),
            _ => None,
        }
    ...
    #[inline]
    fn evaluate_is_null(value: GraphRecordValue) -> Option<GraphRecordValue> {
        match value {
            GraphRecordValue::Null => Some(value),
            _ => None,
        }
...
impl<O: RootOperand> SingleValueWithoutIndexOperation<O> {
    pub(crate) fn evaluate_grouped<'a>(
        &self,
        graphrecord: &'a GraphRecord,
        values: GroupedIterator<'a, Option<GraphRecordValue>>,
    ) -> GraphRecordResult<GroupedIterator<'a, Option<GraphRecordValue>>>
    where
        O: 'a,
    {
        Ok(match self {
