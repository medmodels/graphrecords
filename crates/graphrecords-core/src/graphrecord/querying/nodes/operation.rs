...
use crate::{
    ...
    graphrecord::{
        GraphRecord, GraphRecordAttribute, GraphRecordValue, Group, HandleLookup, NodeHandle,
        NodeIndex,
        datatypes::{
...
impl NodeOperation {
    ...
    ) -> GraphRecordResult<BoxedIterator<'a, NodeHandle>> {
        Ok(match self {
            Self::Values { operand } => {
                Box::new(Self::evaluate_values(graphrecord, node_indices, operand)?)
            }
            ...
            Self::Indices { operand } => {
                Box::new(Self::evaluate_indices(graphrecord, node_indices, operand)?)
            }
            ...
            Self::EitherOr { either, or } => {
                ...
                let either_set: HashSet<NodeHandle> = either
                    .evaluate_forward(graphrecord, Box::new(node_indices_1))?
                    .collect();
                let or_set: HashSet<NodeHandle> = or
                    .evaluate_forward(graphrecord, Box::new(node_indices_2))?
                    .collect();
            ...
            Self::GroupBy { operand } => {
                Box::new(Self::evaluate_group_by(graphrecord, node_indices, operand)?)
            }
        })
    }
    ...
    pub(crate) fn get_values(
        ...
        node_indices: impl Iterator<Item = NodeHandle>,
        attribute: GraphRecordAttribute,
    ) -> impl Iterator<Item = (NodeHandle, GraphRecordValue)> {
    ...
    fn evaluate_values<'a, T>(
        ...
        node_indices: T,
        operand: &Wrapper<MultipleValuesWithIndexOperand<NodeOperand>>,
    ) -> GraphRecordResult<impl Iterator<Item = NodeHandle> + use<'a, T>>
    where
        T: Iterator<Item = NodeHandle> + 'a,
    {
        ...
        let values = Self::get_values(graphrecord, node_indices, attribute.clone());
    ...
    pub(crate) fn get_attributes(
        graphrecord: &GraphRecord,
        node_indices: impl Iterator<Item = NodeHandle>,
    ) -> impl Iterator<Item = (NodeHandle, Vec<GraphRecordAttribute>)> {
        ...
        T: Iterator<Item = NodeHandle>,
    {
        ...
        let result_indices = node_indices_2
            .into_iter()
            .filter(move |index| result.contains(index));

        Ok(result_indices.map(|index| graphrecord.handle_of(&index).expect("Index must exist")))
    }
    ...
    fn evaluate_in_group(
        ...
        node_indices.filter(move |node_index| {
            let groups_of_node = graphrecord
            ...
            match &group {
                ...
                CardinalityWrapper::Multiple(groups, match_mode) => {
                    ...
                    match match_mode {
                        MatchMode::Any => groups.iter().any(|group| groups_of_node.contains(group)),
                        MatchMode::All => groups.iter().all(|group| groups_of_node.contains(group)),
                    }
                }
            }
        })
    ...
    fn evaluate_has_attribute(
        ...
        node_indices.filter(move |node_index| {
            let attributes_view = graphrecord
                .node_attributes(node_index)
                .expect("Node must exist");
            let attributes_of_node: GrHashSet<_> = attributes_view.keys().collect();
                ...
                CardinalityWrapper::Multiple(attributes, match_mode) => {
                    ...
                    match match_mode {
                        MatchMode::Any => attributes
                            .iter()
                            .any(|attribute| attributes_of_node.contains(&attribute)),
        ...
        T: Iterator<Item = NodeHandle>,
    {
        ...
        let edge_indices: RoaringBitmap = operand
            .evaluate_forward(graphrecord, edge_indices)?
            .collect();

        Ok(node_indices_2.filter(move |node_index| {
            let connected_indices: RoaringBitmap = match direction {
                EdgeDirection::Incoming => graphrecord
                    .incoming_edges(node_index)
                    .expect("Node must exist")
                    .collect(),
                EdgeDirection::Outgoing => graphrecord
                    .outgoing_edges(node_index)
                    .expect("Node must exist")
                    .collect(),
                EdgeDirection::Both => graphrecord
                    .incoming_edges(node_index)
                    .expect("Node must exist")
                    .chain(
                        graphrecord
...
impl NodeOperation {
    pub(crate) fn evaluate_grouped<'a>(
        &self,
        graphrecord: &'a GraphRecord,
        node_indices: GroupedIterator<'a, BoxedIterator<'a, NodeHandle>>,
    ) -> GraphRecordResult<GroupedIterator<'a, BoxedIterator<'a, NodeHandle>>> {
        Ok(match self {
            Self::Values { operand } => {
    ...
    fn evaluate_values_grouped<'a>(
        ...
        let values: Vec<_> = node_indices
            .map(|(key, node_indices)| {
                (
                        key,
                        Box::new(Self::get_values(graphrecord, node_indices, attribute.clone()))
    ...
    fn evaluate_attributes_grouped<'a>(
        ...
        let attributes = node_indices.map(|(key, node_indices)| {
            (
                key,
                Box::new(Self::get_attributes(graphrecord, node_indices))
    ...
    fn evaluate_indices_grouped<'a>(
        ...
        let node_indices: GroupedIterator<'a, BoxedIterator<'a, NodeIndex>> =
            Box::new(node_indices.map(|(key, node_handles)| {
                let node_indices = node_handles
                    .map(|handle| {
                        graphrecord
                            .resolve_handle(handle)
                            .expect("Handle must exist")
                ...
                (key, Box::new(node_indices) as BoxedIterator<_>)
            }));

        let (node_indices_1, node_indices_2) = tee_grouped_iterator(node_indices);

        let node_indices_1 = node_indices_1
            .map(|(key, node_indices)| (key, Box::new(node_indices) as BoxedIterator<_>));

        let mut node_indices_1: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, Box::new(node_indices_1))?
            .collect();

        let result_node_indices = Box::new(node_indices_2.map(move |(key, node_indices)| {
        ...
        Ok(Box::new(result_node_indices.map(|(key, node_indices)| {
            let node_handles = node_indices.map(|node_index| {
                graphrecord
                    .handle_of(&node_index)
                    .expect("Index must exist")
            });

            (key, Box::new(node_handles) as BoxedIterator<_>)
        })))
    }
    ...
    fn evaluate_edges_grouped<'a>(
        ...
        let mut edge_indices: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, Box::new(edge_indices))?
            .collect();
    ...
    fn evaluate_neighbors_grouped<'a>(
        ...
        let mut neighbor_indices: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, Box::new(neighbor_indices))?
            .collect();
    ...
    fn evaluate_either_or_grouped<'a>(
        ...
        let either_indices = either.evaluate_forward_grouped(graphrecord, node_indices_1)?;
        let mut or_indices: Vec<_> = or
            .evaluate_forward_grouped(graphrecord, node_indices_2)?
            .collect();
        ...
        let node_indices = either_indices.map(move |(key, either_indices)| {
            ...
            let node_indices: BoxedIterator<_> = Box::new(
                either_indices
                    .chain(or_indices)
                    .unique_by(|node_index| *node_index),
            );
    ...
    fn evaluate_exclude_grouped<'a>(
        ...
        let mut result: Vec<_> = operand
            .evaluate_forward_grouped(graphrecord, node_indices_1)?
            .collect();
...
impl NodeIndicesOperation {
    pub(crate) fn evaluate<'a>(
        ...
        Ok(match self {
            Self::NodeIndexOperation { operand } => {
                Self::evaluate_node_index_operation(graphrecord, indices, operand)?
            }
            Self::NodeIndexComparisonOperation { operand, kind } => {
                Self::evaluate_node_index_comparison_operation(graphrecord, indices, operand, kind)?
            }
            Self::NodeIndicesComparisonOperation { operand, kind } => {
        ...
        T: Iterator<Item = NodeIndex>,
    {
        ...
        Ok(indices.collect::<GraphRecordResult<Vec<_>>>()?.into_iter())
    }
    ...
    fn evaluate_is_string(
        ...
        indices.filter(|index| matches!(index, GraphRecordAttribute::String(_)))
    }
    ...
    fn evaluate_is_int(
        ...
        indices.filter(|index| matches!(index, GraphRecordAttribute::Int(_)))
    }
...
impl NodeIndicesOperation {
    ...
    fn evaluate_node_index_operation_grouped<'a>(
        ...
        let node_indices_1 =
            operand.evaluate_forward_grouped(graphrecord, Box::new(node_indices_1.into_iter()))?;
...
impl NodeIndexOperation {
    pub(crate) fn evaluate(
        ...
        Ok(match self {
            ...
            Self::BinaryArithmeticOperation { operand, kind } => {
                Self::evaluate_binary_arithmetic_operation(graphrecord, node_index, operand, kind)?
            }
            Self::UnaryArithmeticOperation { kind } => {
            ...
            Self::EitherOr { either, or } => {
                Self::evaluate_either_or(graphrecord, node_index, either, or)?
            }
            Self::Exclude { operand } => {
                let result = operand
    ...
    fn evaluate_either_or(
        ...
        let either_result = either.evaluate_forward(graphrecord, Some(index.clone()))?;
...
impl NodeIndexOperation {
    ...
    fn evaluate_either_or_grouped<'a>(
        ...
        let either_indices =
            either.evaluate_forward_grouped(graphrecord, Box::new(node_indices_1))?;
        let mut or_indices: Vec<_> = or
            .evaluate_forward_grouped(graphrecord, Box::new(node_indices_2))?
            .collect();
