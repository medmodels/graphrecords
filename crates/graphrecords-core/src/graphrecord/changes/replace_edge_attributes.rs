...
impl Change for ReplaceEdgeAttributes {
    fn apply(self: Box<Self>, mut state: GraphState) -> GraphRecordResult<GraphState> {
        ...
        for edge_index in edge_indices {
            ...
            state.replace_edge_attributes(address, &attributes)?;
        }
    ...
    fn dispatch(
        ...
        plugin.on_replace_edge_attributes(record, *self)
    }
    ...
    fn post_dispatch_hook(
        ...
        |plugin, previous, candidate| plugin.post_replace_edge_attributes(previous, candidate)
    }
}
